...
app.get('/student/:id', (req, res) => {
    ...
    const data = spiData[spiData.findIndex(temp => temp.id == id)]
...
//Update Student Data
app.patch("/updatastudent/:id", (req, res) => {
    const id = req.params.id;

    const index = spiData.findIndex(temp => temp.id == id);

    spiData[index] = req.body;

    res.send(spiData[index]);
})
...
app.get('/language/:lan',(req,res)=>{
    ...
    const data = spiData.filter(temp => temp.language == lan)
...
//Sindhi language and name starts with A
app.get('/language/:lan/starts/:char',(req,res)=>{
    const lan = req.params.lan;
    const char = req.params.char.toLowerCase();

    const data = spiData.filter(temp => (temp.language == lan) && (temp.name.toLowerCase.startsWith(char)))
