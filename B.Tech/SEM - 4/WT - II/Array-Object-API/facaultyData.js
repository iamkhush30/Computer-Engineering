...
// task 3: print faculties which name starts with A
// task 4: print faculties who is born before 1980
...
//Get Facaulty By ID
app.get('/facaulty/:id', (req, res) => {
    const id = req.params.id;

    const data = faculties[faculties.findIndex(temp => temp.id == id)]
...
//Insert Facaulty Data
app.post('/insertFacaulty', (req, res) => {
...
//Update Facaulty Data
app.patch("/updataFacaulty/:id", (req, res) => {
    const id = req.params.id;

    const index = faculties.findIndex(temp => temp.id == id);

    faculties[index] = req.body;

    res.send(faculties[index]);
})

//Delete Facaulty Data
...
//name starts with A
app.get('/starts/:char',(req,res)=>{
    const char = req.params.char;

    const data = faculties.filter(temp => temp.FacultyName.toLowerCase().startsWith(char.toLowerCase()))
...
//faculties who is born before 1980
app.get('/year/:year',(req,res)=>{
    const year = Number(req.params.year);

    const data = faculties.filter(temp => new Date(temp.FacultyDateOfBirth).getFullYear() < year)
