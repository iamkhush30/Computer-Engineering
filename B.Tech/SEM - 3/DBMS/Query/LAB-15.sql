...
begin
	if(@var1 % 2 = 0)
		return cast(@var1 as varchar(10))+' is Even';
	return cast(@var1 as varchar(10))+' is Odd';
end
...
as
begin
	return case when @var1 > @var2 then cast(@var1 as varchar(10)) +' is Greater'
				when @var1 < @var2 then cast(@var2 as varchar(10)) +' is Greater'
...
alter function oneToN(@var1 int)
returns varchar(50)
as
begin
	declare @i int = 1,@ans varchar(250);
	while @i <= @var1
	begin
		set @ans = CONCAT(@ans+'  '+cast(@i as varchar))
		set @i = @i + 1
	end
	return @ans;
end
